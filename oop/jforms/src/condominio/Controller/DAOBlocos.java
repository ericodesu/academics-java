...
import condominio.Model.ModelBloco;
...
public class DAOBlocos implements CRUDable<ModelBloco> {
    ...
    public void Inserir(ModelBloco modelo) {
        String query = "INSERT INTO blocos (descricao,qtde_andares,qtdes_apartamento_andares) VALUES (?,?,?)";
    ...
    public void Alterar(ModelBloco modelo) {
        String query = "UPDATE blocos SET descricao=?,qtde_andares=?,qtdes_apartamento_andares=? WHERE id_bloco=?";
	...
	String query = "SELECT * FROM blocos";
	    ...
	    while(this._rs.next()) {
	        ModelBloco modelo = new ModelBloco();
                ...
                modelo.setQuantidadeApartamentoAndares(this._rs.getInt("qtdes_apartamento_andares"));
